...
class VocabEntry(object):
    ...
    def to_input_tensor(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        return torch.t(sents_var) # 求转置，即[N, L] -> [L, N]
    ...
    def from_corpus(corpus, size, freq_cutoff=2):
        ...
        top_k_words = sorted(valid_words, key=lambda w: word_freq[w], reverse=True)[:size] # 限定上限vocabulary size
...
if __name__ == '__main__':
    ...
    src_sents = read_corpus(args['--train-src'], source='src') # list
    tgt_sents = read_corpus(args['--train-tgt'], source='tgt') # list

    vocab = Vocab.build(src_sents, tgt_sents, int(args['--size']), int(args['--freq-cutoff']))
    print('generated vocabulary, source %d words, target %d words' % (len(vocab.src), len(vocab.tgt)))

    vocab.save(args['VOCAB_FILE']) # 存下词典
