...
class CharDecoder(nn.Module):
    ...
    def forward(self, input, dec_hidden=None):
        ...
        char_embedding = self.decoderCharEmb(input) # [length, batch_size, char_embed_size]
        hiddens, dec_hidden = self.charDecoder(char_embedding, dec_hidden) # [length, batch_size, hidden_size], ([1, batch, hidden_size], [1, batch, hidden_size])
        scores = self.char_output_projection(hiddens) # [length, batch_size, char_vocab_size]
    ...
    def train_forward(self, char_sequence, dec_hidden=None):
        ...
        input = char_sequence[:-1] # input with out the END token
        target = char_sequence[1:].contiguous().view(-1) # target size should be (N,)
        loss_function = nn.CrossEntropyLoss(ignore_index = self.target_vocab.char_pad, reduction = 'sum')

        scores, dec_hidden = self.forward(input, dec_hidden) # (length, batch_size, self.vocab_size), ((1, batch, hidden_size), (1, batch, hidden_size))
        scores = scores.view(-1, scores.shape[-1]) # scores size should be (N, C)
    ...
    def decode_greedy(self, initialStates, device, max_length=21):
        ...
        input = torch.tensor([[start_idx] for i in range(batch_size)], device = device).transpose(1, 0) # [1, batch_size]
        output = [["", False] for i in range(batch_size)]

        for i in range(max_length):
            scores, dec_hidden = self.forward(input, dec_hidden) # [1, batch_size, self.vocab_size]
            input = scores.argmax(dim = 2) # [1, batch_size]

            for batch_idx, vocab_idx in enumerate(input.detach().squeeze(0)):
                if(not output[batch_idx][1]):
                    if(vocab_idx == end_idx):
                        output[batch_idx][1] = True
                    else:
                        output[batch_idx][0] += self.target_vocab.id2char[vocab_idx.item()]

        decodedWords = [each[0] for each in output]
        return decodedWords
