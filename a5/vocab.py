...
class VocabEntry(object):
    ...
    def __init__(self, word2id=None):
        ...
        self.char_list = list(
            """ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789,;.!?:'\"/\\|_@#$%^&*~`+-=<>()[]""") # char_list是原定好的
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        charindices = self.words2charindices(sents) # list of list of list of char_id
        charindices_padded = pad_sents_char(charindices, self.char_pad) # (batch_size, max_sentence_length, max_word_length)
        sents_var = torch.LongTensor(charindices_padded).permute(1, 0, 2).contiguous() # [max_sentence_length, batch_size, max_word_length]

        return sents_var.to(device)
    ...
    def to_input_tensor(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        """
        word_ids = self.words2indices(sents) # list of list of int
...
class Vocab(object):
    ...
    def save(self, file_path):
        ...
        """
        json.dump(dict(src_word2id=self.src.word2id, tgt_word2id=self.tgt.word2id), open(file_path, 'w'), indent=2) # 只保存了word vocab
