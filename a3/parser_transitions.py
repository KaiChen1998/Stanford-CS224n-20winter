...
class PartialParse(object):
    def __init__(self, sentence):
        ...
        self.stack = ["ROOT"]
        self.buffer = list(sentence) # should create a new list for buffer instead of directly pointing to the original sentence object
        self.dependencies = []
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) > 0:
        ...
        for pp, transition in zip(batch_parses, batch_transitions):
            pp.parse([transition])
            
            if(len(pp.stack) == 1 and len(pp.buffer) == 0):
                unfinished_parses.remove(pp) # only remove the index in this list                

    dependencies = [each.dependencies for each in partial_parses]
