...
class Parser(object):
    ...
    def __init__(self, dataset):
        ...
        tok2id = {L_PREFIX + l: i for (i, l) in enumerate(deprel)} # <l> + dependence & <l> + <NULL>
        ...
        self.tran2id = {t: i for (i, t) in enumerate(trans)} # number classes
    ...
    def extract_features(self, stack, buf, arcs, ex):
        ...
        def get_lc(k):
            return sorted([arc[1] for arc in arcs if arc[0] == k and arc[1] < k]) # 取直接子节点
        ...
        p_features = []
        l_features = []
        features = [self.NULL] * (3 - len(stack)) + [ex['word'][x] for x in stack[-3:]] # stack top 3 words
        features += [ex['word'][x] for x in buf[:3]] + [self.NULL] * (3 - len(buf)) # buffer top 3 words
        if self.use_pos:
        ...
        features += p_features + l_features # 串联
        ...
        with tqdm(total=len(dataset)) as prog:
            for i, ex in enumerate(dataset):
                ...
                for pred_h, gold_h, gold_l, pos in \
                        ...
                        if (self.with_punct) or (not punct(self.language, pos_str)): # 判断标点符号
...
def read_conll(in_file, lowercase=False, max_example=None):
    ...
    with open(in_file) as f:
        ...
        for line in f.readlines():
            sp = line.strip().split('\t')
            if len(sp) == 10: # normal word
                if '-' not in sp[0]:
                    word.append(sp[1].lower() if lowercase else sp[1])
                    pos.append(sp[4])
                    head.append(int(sp[6]))
                    label.append(sp[7])
            elif len(word) > 0: # empty line
...
def load_and_preprocess_data(reduced=True):
    ...
    embeddings_matrix = np.asarray(np.random.normal(0, 0.9, (parser.n_tokens, 50)), dtype='float32') # embedding dimension = 50
