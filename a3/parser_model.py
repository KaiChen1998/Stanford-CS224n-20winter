...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        @param embeddings (ndarray): word embeddings (num_words, embedding_size) (D, |V|)
        ...
        self.embeddings = nn.Parameter(torch.tensor(embeddings)) # not fix embedding
    ...
    def forward(self, w):
        ...
        x = x.matmul(self.embed_to_hidden_weight) + self.embed_to_hidden_bias
        x = F.relu(x)
        x = self.dropout(x)
        logits = x.matmul(self.hidden_to_logits_weight) + self.hidden_to_logits_bias
