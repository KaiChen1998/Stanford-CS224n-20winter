...
class StanfordSentiment:
    ...
    def tokens(self):
        ...
        tokens = dict() # word to idx
        tokenfreq = dict() # frequency
        wordcount = 0 # 计重复的总字数
        revtokens = [] # idx to word
        idx = 0 # 不计重复的总字数
    ...
    def sentences(self):
        ...
        self._sentences = sentences # list of list of str(小写)
        self._sentlengths = np.array([len(s) for s in sentences])
        self._cumsentlen = np.cumsum(self._sentlengths) # 累计长度
    ...
    def allSentences(self):
        ...
        # 排除两种情况：
        # 1. 总出现次数少于总次数的1e-5
        # 2. 每次随机一个数，如果低于其reject probability（频率越高，reject概率越高）
        allsentences = [[w for w in s
            if 0 >= rejectProb[tokens[w]] or random.random() >= rejectProb[tokens[w]]] # 应对频繁项，频繁项出现的概率低
    ...
    def getRandomContext(self, C=5):
        ...
        sent = allsent[sentID] # 选一句话
        wordID = random.randint(0, len(sent) - 1) # 选一个中心词
    ...
    def sampleTable(self):
        ...
        for w in range(nTokens):
            ...
            if w in self._tokenfreq:
                ...
                # Reweigh
                freq = freq ** 0.75 # P(W) = U(W) ** 0.75
            else:
                freq = 0.0
            samplingFreq[i] = freq
            i += 1

        # 把所有的单词排成一列，所占有的面积是他的频率（概率）
        # table是不过把(0, 1)的粒度变大了而已
